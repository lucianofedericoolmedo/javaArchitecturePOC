-- Function: detallefacturacionporcomprobantesprendas(character varying, character varying, character varying, boolean, character varying, character varying)

-- DROP FUNCTION detallefacturacionporcomprobantesprendas(character varying, character varying, character varying, boolean, character varying, character varying);

CREATE OR REPLACE FUNCTION detallefacturacionporcomprobantesprendas(fdesde character varying, fhasta character varying, cliente character varying, mostrarpuedeutilizarrfid boolean, archivo character varying, separador character varying)
  RETURNS void AS
$BODY$
    DECLARE
	query TEXT;
	...
	query := format('(SELECT datos.*, producto.puede_tener_rfid FROM (
				SELECT remitos_retiro.numero, remitos_retiro.fecha_creacion, productos.id, productos.nombre, SUM(items_remito_retiro.cantidad) AS cantidad 
				FROM remitos_retiro
				...
				SELECT alquileres.numero, DATE(facturaciones_alquileres.fecha_facturacion), productos.id, productos.nombre, SUM(facturaciones_alquileres.cantidad_facturada) AS cantidad 
				...
				SELECT remitos_stock.numero, remitos_stock.fecha_creacion, productos.id, productos.nombre, SUM(items_remito_stock.cantidad) AS cantidad
				FROM remitos_stock
				...
				SELECT ajustes_de_stock.numero, ajustes_de_stock.fecha_creacion, productos.id, productos.nombre, SUM(items_ajustes_de_stock.cantidad) AS cantidad
				...
				SELECT ajustes_de_stock.numero, ajustes_de_stock.fecha_creacion, productos.id, productos.nombre, 0 -SUM(items_ajustes_de_stock.cantidad) AS cantidad
				FROM ajustes_de_stock
					INNER JOIN items_ajustes_de_stock
						ON ajustes_de_stock.id = items_ajustes_de_stock.remito
					INNER JOIN productos
...
$BODY$
  ...
  COST 100;
ALTER FUNCTION detallefacturacionporcomprobantesprendas(character varying, character varying, character varying, boolean, character varying, character varying)
  OWNER TO postgres;
