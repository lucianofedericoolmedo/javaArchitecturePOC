DELIMITER $$
CREATE DEFINER=`root`@`localhost` PROCEDURE `detallefacturacionporprendas`(IN fdesde CHAR(25), IN fhasta CHAR(25), cliente CHAR(50), mostrarpuedeutilizarrfid BOOLEAN, archivo CHAR(75), separador CHAR(5))
BEGIN 
	SET @innerQuery = CONCAT("
		(SELECT datos.id, datos.nombre, SUM(datos.cantidad) AS cantidad, producto.puede_tener_rfid FROM (
